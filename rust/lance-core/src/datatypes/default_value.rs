...
pub enum DefaultValueError {
    ...
    #[snafu(display("Invalid default value literal '{literal}' for type {data_type:?}: {reason}"))]
    InvalidLiteral {
        literal: String,
        data_type: DataType,
        reason: String,
...
fn format_datetime_with_tz(
    ...
    if !col_tz.eq_ignore_ascii_case("utc")
        && col_tz != "Z"
        && let Ok(fo) = col_tz.parse::<chrono::FixedOffset>()
    {
