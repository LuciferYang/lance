...
impl ZoneMapIndexBuilder {
    ...
    ///
    /// Public so external builders (e.g. a Spark driver consolidating per-fragment zone batches
    /// from parallel executor tasks) can extract the trained state without forcing the in-place
    /// file write that [`Self::write_index`] performs. The companion
    /// [`write_zonemap_index_from_batch`] free function consumes the same shape.
