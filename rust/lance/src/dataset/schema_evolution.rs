...
use lance_core::datatypes::{
    Field, LANCE_INITIAL_DEFAULT_META_KEY, LANCE_WRITE_DEFAULT_META_KEY, Schema, decode_default,
...
pub(super) async fn add_columns_to_fragments(
    ...
    let (output_schema, new_fragments, fragments_to_cleanup) = match transforms {
        ...
        NewColumnTransform::AllNulls(output_schema) => {
            ...
            let new_schema = Schema::try_from(output_schema.as_ref())?;
            // AllNulls is metadata-only; missing columns are synthesized at read time, so each
            // new top-level column must be nullable unless it carries an initial-default for the
            // read path to backfill.
            if let Some(field) = new_schema
                .fields
                .iter()
                .find(|f| !f.nullable && f.initial_default_raw().is_none())
            {
                return Err(Error::invalid_input_source(
                    format!(
                        "column '{}': non-nullable columns must carry an initial-default value \
                         (lance-schema:initial-default) when added to an existing dataset; \
                         provide a default or make the column nullable",
                        field.name
                    )
                    .into(),
                ));
            ...
            // Reject an invalid initial-default literal before committing.
