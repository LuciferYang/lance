...
mod test {
    ...
    //
    // We keep memory low by storing one ~128MB value and taking it 17 times via
    // duplicate indices, so the cumulative take output crosses 2GB while the
    // source array stays ~128MB. arrow checks the offset overflow BEFORE
    // allocating the (would-be 2GB) output values buffer, so peak RSS stays well
    // under a GB.
    //
    // #[ignore]: still allocates a ~128MB value plus decode buffers; run manually:
    //   cargo test -p lance take::test::repro_binary_take_offset_overflow_2gb -- --ignored --nocapture
    #[tokio::test]
    ...
    async fn repro_binary_take_offset_overflow_2gb() {
        ...
        // Dataset: a single-row `Binary` column holding one 128MB value.
        ...
        // Expect an Offset overflow error surfaced from arrow's take_bytes.
        match result {
