...
//!
//! [`lance_table::transaction::COLUMN_DEFAULTS_MODEL_CONFIG_KEY`] is the per-dataset manifest
//! flag that selects between **Model A** (single mutable default; the default) and **Model B**
//! (two-value defaults: initial-default + write-default). It is re-exported as
//! [`crate::dataset::transaction::COLUMN_DEFAULTS_MODEL_CONFIG_KEY`].
//!
//! Set it via the existing config-update API:
//!
//! ```ignore
//! dataset.update_config([(COLUMN_DEFAULTS_MODEL_CONFIG_KEY, "B")]).await?;
//! ```
//!
//! `lance_table::transaction` reads the flag while building the next manifest to
//! gate the initial-default immutability guard.

use crate::index::DatasetIndexExt;
use lance_core::datatypes::{LANCE_INITIAL_DEFAULT_META_KEY, validate_default_assignable};
...
/// Names of the "Model A" defaulted columns -- a mutable initial-default and no write-default --
/// that are physically absent from every one of `fragments`.
///
/// Such a column is read-backfilled at whatever the default currently is, so any rewrite that
/// materializes it freezes that value. Callers project it out of the rewrite instead, which is
/// only sound when it is absent from every fragment involved: a column with mixed presence has
/// stored values that must not be dropped.
pub(super) fn model_a_absent_columns<'a>(
    dataset: &'a Dataset,
    fragments: &[lance_table::format::Fragment],
) -> Vec<&'a str> {
    let present: std::collections::HashSet<i32> = fragments
        .iter()
    ...
    pub async fn set_column_default(&mut self, column: &str, default_literal: &str) -> Result<()> {
        ...
        let field = self.schema().field_by_id(field_id).ok_or_else(|| {
            Error::invalid_input(format!(
                "column '{column}': field id {field_id} not found in schema"
            ))
        })?;
...
mod tests {
    ...
    async fn test_model_flag_absent_means_model_a() -> Result<()> {
        ...
        assert!(
            !lance_table::transaction::column_defaults_model_b_enabled(&dataset.manifest),
        ...
        assert!(
            !dataset
                .config()
                .contains_key(lance_table::transaction::COLUMN_DEFAULTS_MODEL_CONFIG_KEY),
    ...
    async fn test_model_flag_set_to_b_persists_and_reader_returns_true() -> Result<()> {
        ...
        dataset
            .update_config([(
                lance_table::transaction::COLUMN_DEFAULTS_MODEL_CONFIG_KEY,
                "B",
            )])
            .await?;
        ...
        assert_eq!(
            dataset
                .config()
                .get(lance_table::transaction::COLUMN_DEFAULTS_MODEL_CONFIG_KEY)
                .map(|s| s.as_str()),
            Some("B"),
    ...
    async fn test_model_flag_survives_reopen() -> Result<()> {
        ...
        assert!(
            lance_table::transaction::column_defaults_model_b_enabled(&reopened.manifest),
...
mod enforcement_tests {
    ...
    /// Build a dataset whose single `x` column is **non-nullable**, so the
    /// unenforced-primary-key designation is legal on it (`verify_primary_key`
    /// rejects a nullable primary key outright).
    async fn make_pk_dataset() -> Dataset {
        let schema = Arc::new(ArrowSchema::new(vec![ArrowField::new(
            "x",
            DataType::Int32,
            false,
    ...
    async fn test_immutability_new_field_may_set_initial_default() -> Result<()> {
        ...
        let new_schema = Arc::new(ArrowSchema::new(vec![
            ArrowField::new("y", DataType::Int32, true).with_metadata(HashMap::from([(
                LANCE_INITIAL_DEFAULT_META_KEY.to_string(),
                "7".to_string(),
            )])),
        ]));
    ...
    async fn test_pk_coprohibition_initial_default_then_pk_key_truthy() -> Result<()> {
        let mut dataset = make_pk_dataset().await;
    ...
    async fn test_pk_coprohibition_fires_on_create() -> Result<()> {
        ...
        let schema = Arc::new(arrow_schema::Schema::new(vec![
            arrow_schema::Field::new("k", arrow_schema::DataType::Int32, false)
                .with_metadata(field_meta),
        ]));
        let batch = arrow_array::RecordBatch::try_new(
            schema.clone(),
...
mod model_b_e2e_tests {
    ...
    use arrow_array::{Array, Int32Array, RecordBatch, RecordBatchIterator, cast::AsArray};
    use arrow_schema::{DataType, Field as ArrowField, Schema as ArrowSchema};
    use lance_core::datatypes::{LANCE_INITIAL_DEFAULT_META_KEY, LANCE_WRITE_DEFAULT_META_KEY};
