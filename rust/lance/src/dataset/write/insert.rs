...
impl<'a> InsertBuilder<'a> {
    ...
    async fn write_uncommitted_stream_impl(
        ...
        // `written_schema` carries the columns that were injected from their write-defaults, so
        // an Overwrite commits a manifest that declares them.
    ...
    fn validate_write(&self, context: &mut WriteContext, data_schema: &Schema) -> Result<()> {
            ...
            && let WriteDestination::Dataset(dataset) = &context.dest
        {
            ...
            // is non-nullable: a write-default is materialised before the data is written, and
            // an initial-default is backfilled on read. `allow_missing_if_nullable` only
            // tolerates missing *nullable* fields, so drop those columns from the schema this
            // pre-write check compares against.
            let normalized_data_schema = prepared_to_logical_blob_schema(data_schema)?;
            let expected_schema =
                super::drop_omitted_defaulted_fields(dataset.schema(), &normalized_data_schema);
            if normalized_data_schema
                .check_compatible(&expected_schema, &schema_cmp_opts)
                .is_err()
            {
                ...
                normalized_data_schema.check_compatible(
                    &super::drop_omitted_defaulted_fields(&dataset_schema, &normalized_data_schema),
                    &schema_cmp_opts,
                )?;
            }
        }
